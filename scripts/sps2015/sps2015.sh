...
max_freq=5000
window_length="$2"
dft_length=`echo $((${window_length}* 2))`
segments="$3"
beta=0.5
n_tests=10
...
do
  ...
  then
    ...
    ./pymir3-cl.py tool tensor_spectrogram -f $segments /tmp/$$.trim "$target_name"
    rm /tmp/$$ /tmp/$$.trim
  fi
done
...
final_eval=`./pymir3-cl.py info evaluation_statistics $evaluations`
echo ${final_eval}
echo ${final_eval} > "${database}"_"${window_length}"_"${segments}".eval

...
do
    ...
    best_score=`./pymir3-cl.py unsupervised detection threshold lower_rhythm_entropy $scorenames`
    final_target="${best_score}.selected"
    cp $best_score $final_target

    target_name3="${basename}.beta.lower_rhythm_entropy.symbolic.eval"
...
evaluations=`find "$database"/Pieces/Audio/ -name "*beta.lower_rhythm_entropy.symbolic.eval"`

...
do
  ...
  do
    ...
    #then
      ...
      evaluation=`./pymir3-cl.py info evaluation_csv -H "$target_name3"`
      # Here, I will calculate MIDI features
      density=`./pymir3-cl.py symbolic density "$target_name2"`
      range=`./pymir3-cl.py symbolic range "$target_name2"`
      pitchclass=`./pymir3-cl.py symbolic pitchclass "$target_name2"`
      intervals=`./pymir3-cl.py symbolic intervals "$target_name2"`
      relativerange=`./pymir3-cl.py symbolic relativerange "$target_name2"`
      rhythm=`./pymir3-cl.py symbolic intervals "$target_name2"`
      full_line=`echo $evaluation $th $density $range $pitchclass $intervals $relativerange $rhythm | tr ' ' ','`
      #echo $evaluation
      #echo $th
      #echo $density
      #echo $range
...
#evaluations=`find "$database"/Pieces/Audio/ -name "*${final_th}.beta.symbolic.eval"`

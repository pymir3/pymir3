...
do
  ...
  then
    ...
    ./pymir3-cl.py supervised linear extract right /tmp/$$ /tmp/$$.nofilter
    ./pymir3-cl.py unsupervised filtering median 2 /tmp/$$.nofilter /tmp/$$.medianfilter
    ./pymir3-cl.py unsupervised filtering hard-sparsity 8 /tmp/$$.medianfilter "$target_name"
    rm /tmp/$$
    rm /tmp/$$.medianfilter
    rm /tmp/$$.nofilter
  fi
...
do
  ...
  if [ ! -e "$target_name1" ]
    then
      ./pymir3-cl.py unsupervised detection hmm detect 0.1 "$name" "$target_name1"
  fi
